...
class NUSNextBusService {
    ...
    private func makeRequest(endpoint: String) -> URLRequest? {
        guard let url = URL(string: "\(NetworkConstants.baseURL)\(endpoint)") else { return nil }
        var request = URLRequest(url: url)
        let authString = "\(NetworkConstants.username):\(NetworkConstants.password)"    // authentication
...
//        guard let request = makeRequest(endpoint: "/ActiveBus?route_code=\(routeCode)") else { return }
//        
//        URLSession.shared.dataTask(with: request) { data, _, error in
//            if let error = error { return completion(.failure(error)) }
