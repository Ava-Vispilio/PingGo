...
@MainActor
class WatchArrivalViewModel: ObservableObject {
    ...
    func fetchArrivalTimes(for stopId: String) async {
        ...
        do {
            print("[Watch] Fetching arrival times for stop ID: \(stopId)")
    ...
    func toggleNotification(_ enabled: Bool) {
        ...
        if enabled {
            print("[Watch] Enabling notification for stop \(stopId)")
            scheduleNotification(id: notificationId)
        } else {
            print("[Watch] Cancelling notification for stop \(stopId)")
    ...
    func rescheduleNotificationIfNeeded() {
        ...
        let notificationId = "bus_arrival_\(stopId)"
        print("[Watch] Rescheduling notification for stop \(stopId)")
        notificationManager.cancelNotification(id: notificationId)
        scheduleNotification(id: notificationId)
    }
